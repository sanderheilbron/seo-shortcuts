...
chrome.omnibox.onInputChanged.addListener(function(text, suggest) {
  ...
  function filteredResults (suggestions, searchQuery) {
    if (searchQuery !== "" ) { // to avoid to show the first 5 commands from the commands array as default
  ...
  function getDescriptionForMatchedCommand(command, arrayOfCommands) {
    return arrayOfCommands.filter(function(result) {
      return (result.content === command);
    }).map(function(result) {
      const originalDescription = result.description;
      const modifiedDescription = originalDescription.replace(/<match>(.*)<\/match>/, '<url><match>$1</match></url>');
...
chrome.omnibox.onInputEntered.addListener(function(text, tab) {
  if (text !== 'help') {
    chrome.tabs.executeScript(tab.id, {file: "js/commands/"+ text +".js"});
  } else {
