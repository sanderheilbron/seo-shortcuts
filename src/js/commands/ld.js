...
(function() {
    ...
    const el = Array.from(document.querySelectorAll('[type="application/ld+json"]'));
    let count = 0;
    console.info(`Found ${el.length} JSON-LD script element${(el.length === 1) ? "" : "s"}.`);
    ...
    el.forEach.call(el, (element) => {
        ...
        console.groupCollapsed(`%cScript element #${count} - ${itemScope}`, "color:blue");
