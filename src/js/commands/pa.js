/**
 * pa.js 1.0
 * Shows a performance analysis of the current page through the Resource Timing API, Navigation Timing API and User-Timing
...
})(
  {
      ...
      function(require, module, exports) {
        ...
        */
        ...
        legendComponent.init = function() {
          ...
          legendsHolder.appendChild(
            createLegend(
              'initiator-type-legend',
          ...
          legendsHolder.appendChild(
            createLegend('navigation-legend', 'Navigation Timing', [
              ['Redirect', '#ffff60'],
              ['App Cache', '#1f831f'],
              ['DNS Lookup', '#1f7c83'],
              ['TCP', '#e58226'],
          ...
          legendsHolder.appendChild(
            createLegend('resource-legend', 'Resource Timing', [
        ...
        navigationTimelineComponent.init = function() {
          ...
          perfTimingCalc.blocks = [
            waterfall.timeBlock(
              'Total',
              0,
              perfTimingCalc.pageLoadTime,
              'block-total',
            ),
            waterfall.timeBlock(
              'Unload',
              perfTimingCalc.unloadEventStart,
              perfTimingCalc.unloadEventEnd,
              'block-unload',
            ),
            waterfall.timeBlock(
              'Redirect (' + performance.navigation.redirectCount + 'x)',
              perfTimingCalc.redirectStart,
              perfTimingCalc.redirectEnd,
              'block-redirect',
            ),
            waterfall.timeBlock(
              'App cache',
              perfTimingCalc.fetchStart,
              perfTimingCalc.domainLookupStart,
              'block-appcache',
            ),
            waterfall.timeBlock(
              'DNS',
              perfTimingCalc.domainLookupStart,
              perfTimingCalc.domainLookupEnd,
              'block-dns',
            ),
            waterfall.timeBlock(
              'TCP',
              perfTimingCalc.connectStart,
              perfTimingCalc.connectEnd,
              'block-tcp',
            ),
            waterfall.timeBlock(
              'Time to First Byte',
              perfTimingCalc.requestStart,
              perfTimingCalc.responseStart,
              'block-ttfb',
            ),
            waterfall.timeBlock(
              'Response',
              perfTimingCalc.responseStart,
              perfTimingCalc.responseEnd,
              'block-response',
            ),
            waterfall.timeBlock(
              'DOM Processing',
              perfTimingCalc.domLoading,
              perfTimingCalc.domComplete,
              'block-dom',
            ),
            waterfall.timeBlock(
              'domContentLoaded Event',
              perfTimingCalc.domContentLoadedEventStart,
              perfTimingCalc.domContentLoadedEventEnd,
              'block-dom-content-loaded',
            ),
            waterfall.timeBlock(
              'onload Event',
              perfTimingCalc.loadEventStart,
              perfTimingCalc.loadEventEnd,
              'block-onload',
            ),
          ];

          if (perfTimingCalc.secureConnectionStart) {
            perfTimingCalc.blocks.push(
              waterfall.timeBlock(
                'SSL',
                perfTimingCalc.connectStart,
                perfTimingCalc.secureConnectionStart,
                'block-ssl',
              ),
            );
          }
          ...
          if (
            !perfTimingCalc.redirectEnd &&
            !perfTimingCalc.redirectStart &&
            perfTimingCalc.fetchStart > perfTimingCalc.navigationStart
          ) {
            perfTimingCalc.blocks.push(
              waterfall.timeBlock(
                'Cross-Domain Redirect (and/or other Delay)',
                perfTimingCalc.navigationStart,
                perfTimingCalc.fetchStart,
                'block-redirect',
              ),
            );
          ...
          data.measures.forEach(function(measure) {
            perfTimingCalc.blocks.push(
              waterfall.timeBlock(
                ...
                Math.round(measure.startTime),
                Math.round(measure.startTime + measure.duration),
          ...
          return waterfall.setupTimeLine(
            Math.round(perfTimingCalc.pageLoadTime),
            perfTimingCalc.blocks,
            data.marks,
            [],
            'Navigation Timing',
      ...
      function(require, module, exports) {
        ...
        */
        ...
        pageMetricComponent.init = function() {
          ...
          var persistDataCheckboxLabel = dom.newTag('label', {
            text: ' Persist Data?',
          });
          ...
          var printDataButton = dom.newTag('button', {
          ...
          persistDataCheckboxLabel.insertBefore(
            persistDataCheckbox,
            persistDataCheckboxLabel.firstChild,
          );
    ...
    4: [
      function(require, module, exports) {
        ...
        */
        ...
        var pieChartHelpers = _interopRequire(
          require('../helpers/pieChartHelpers'),
        );
        ...
        pieChartComponent.init = function() {
          ...
          var setupChart = function(title, chartData, countTexts, columns, id) {
            ...
            chartHolder.appendChild(
              pieChartHelpers.createPieChart(chartData, 400),
            );
            chartHolder.appendChild(
              dom.newTag('p', {
                text: 'Total Requests: ' + data.requestsOnly.length,
              }),
            );
            ...
            chartHolder.appendChild(
              pieChartHelpers.createChartTable(title, chartData, columns),
            );
          ...
          //argument data
          var requestsByDomainData = data.requestsByDomain.map(function(
            sourceDomain,
          ) {
            ...
            if (domain.domain === location.host) {
              ...
              domain.colour = helper.getRandomColor(
                '56789abcdef',
                '01234567',
                'abcdef',
              );
            ...
            domain.id =
              'reqByDomain-' + domain.label.replace(/[^a-zA-Z]/g, '-');
            domain.durationAverage = Math.round(
              domain.durationTotal / domain.count,
            );
            domain.durationTotal = Math.round(domain.durationTotal);
            domain.durationTotalParallel = Math.round(
              domain.durationTotalParallel,
            );
            return domain;
          });
          ...
          setupChart(
            ...
            data.initiatorTypeCounts.map(function(initiatorype) {
              initiatorype.perc = initiatorype.count / requestsUnit;
              initiatorype.label = initiatorype.initiatorType;
              initiatorype.colour = helper.getInitiatorOrFileTypeColour(
                initiatorype.initiatorType,
                helper.getRandomColor(colourRangeR, colourRangeG, colourRangeB),
              );
              initiatorype.id =
                'reqByInitiatorType-' +
                initiatorype.label.replace(/[^a-zA-Z]/g, '-');
              return initiatorype;
            }),
          );
          ...
          setupChart(
            ...
            data.initiatorTypeCountHostExt.map(function(initiatorype) {
          ...
          setupChart(
            ...
            data.fileTypeCounts.map(function(fileType) {
              fileType.perc = fileType.count / requestsUnit;
              fileType.label = fileType.fileType;
              fileType.colour = helper.getInitiatorOrFileTypeColour(
                fileType.fileType,
                helper.getRandomColor(colourRangeR, colourRangeG, colourRangeB),
              );
              fileType.id =
                'reqByFileType-' + fileType.label.replace(/[^a-zA-Z]/g, '-');
              return fileType;
            }),
          );
          ...
          setupChart(
            ...
            data.fileTypeCountHostExt.map(function(fileType) {
      ...
      function(require, module, exports) {
        ...
        */
        ...
        var getChartData = function getChartData(filter) {
          var calc = {
            pageLoadTime:
              data.perfTiming.loadEventEnd - data.perfTiming.responseStart,
            lastResponseEnd:
              data.perfTiming.loadEventEnd - data.perfTiming.responseStart,
          };
          ...
          for (var perfProp in data.perfTiming) {
            if (
              ...
              calc[perfProp] =
                data.perfTiming[perfProp] - data.perfTiming.navigationStart;
            }
          }

          var onDomLoad = waterfall.timeBlock(
            'domContentLoaded Event',
            calc.domContentLoadedEventStart,
            calc.domContentLoadedEventEnd,
            'block-dom-content-loaded',
          );
          var onLoadEvt = waterfall.timeBlock(
            'Onload Event',
            calc.loadEventStart,
            calc.loadEventEnd,
            'block-onload',
          ...
          var navigationApiTotal = [
            ...
            waterfall.timeBlock(
              'App cache',
              calc.fetchStart,
              calc.domainLookupStart,
              'block-appcache',
            ),
            waterfall.timeBlock(
              'DNS',
              calc.domainLookupStart,
              calc.domainLookupEnd,
              'block-dns',
            ),
            waterfall.timeBlock(
              'TCP',
          ...
          if (calc.secureConnectionStart) {
            navigationApiTotal.push(
              waterfall.timeBlock(
                'SSL',
                calc.connectStart,
                calc.secureConnectionStart,
                'block-ssl',
              ),
            );
          }
          ...
          if (
            !calc.redirectEnd &&
            !calc.redirectStart &&
            calc.fetchStart > calc.navigationStart
          ) {
            navigationApiTotal.push(
              waterfall.timeBlock(
                'Cross-Domain Redirect',
                calc.navigationStart,
                calc.fetchStart,
                'block-redirect',
              ),
            );
          ...
          data.allResourcesCalc
            ...
            .forEach(function(resource, i) {
              var segments = [
                ...
                waterfall.timeBlock(
                  'secureConnect',
                  resource.secureConnectionStart || undefined,
                  resource.connectEnd,
                  'block-ssl',
                ),
                waterfall.timeBlock(
              ...
              var firstTiming = resourceTimings.reduce(function(
                currMinTiming,
                currentValue,
              ) {
                if (
                  currentValue > 0 &&
                  (currentValue < currMinTiming || currMinTiming <= 0) &&
                  currentValue != resource.startTime
                ) {
                  return currentValue;
                } else {
              ...
              calc.blocks.push(
                waterfall.timeBlock(
                  ...
                  resource.startTime,
                  resource.responseEnd,
                  'block-' + resource.initiatorType,
                  segments,
                  resource,
                ),
              );
              calc.lastResponseEnd = Math.max(
                calc.lastResponseEnd,
                resource.responseEnd,
              );
            });
        ...
        resourcesTimelineComponent.init = function() {
          ...
          var chartHolder = waterfall.setupTimeLine(
          ...
          if (data.requestsByDomain.length > 1) {
            var selectBox = dom.newTag('select', {
              ...
              onchange: function onchange() {
                ...
                var tempChartHolder = waterfall.setupTimeLine(
                  chartData.loadDuration,
                  chartData.blocks,
                  data.marks,
                  chartData.bg,
                ...
                var oldSVG = chartHolder.getElementsByClassName(
                  'water-fall-chart',
                )[0];
                var newSVG = tempChartHolder.getElementsByClassName(
                  'water-fall-chart',
                )[0];
            ...
            var chartSvg = chartHolder.getElementsByClassName(
              'water-fall-chart',
            )[0];
      ...
      function(require, module, exports) {
        ...
        */
        ...
        summaryTilesComponent.init = function() {
          ...
          var createAppendixDefValue = function createAppendixDefValue(
            a,
            definition,
            value,
          ) {
          ...
          [
            ...
            createTile(
              ...
              data.currAndSubdomainRequests || '0',
            ),
            createTile(
              'Total',
              data.perfTiming.loadEventEnd -
                data.perfTiming.navigationStart +
                'ms',
              40,
            ),
            createTile(
              'Time to First Byte',
              data.perfTiming.responseStart -
                data.perfTiming.navigationStart +
                'ms',
              40,
            ),
            createTile(
              ...
              data.perfTiming.domContentLoadedEventStart -
                data.perfTiming.domLoading +
                'ms',
              40,
            ),
            createTile(
              ...
              data.perfTiming.domComplete - data.perfTiming.domLoading + 'ms',
              40,
            ),
          ...
          if (data.allResourcesCalc.length > 0) {
            tilesHolder.appendChild(
              createTile(
                dom.newTag('span', {
                  title: data.slowestCalls[0].name,
                  text: 'Slowest Call',
                }),
                dom.newTag('span', {
                  title: data.slowestCalls[0].name,
                  text: Math.floor(data.slowestCalls[0].duration) + 'ms',
                }),
                40,
              ),
            );
            tilesHolder.appendChild(
              createTile('Average Call', data.average + 'ms', 40),
            );
          }
          ...
          createAppendixDefValue(
            appendix,
            dom.newTextNode('Host:'),
            location.host,
          );
          createAppendixDefValue(
            appendix,
            dom.newTextNode('document.domain:'),
            document.domain,
          );
        ...
        tableComponent.init = function() {
          var output = data.requestsOnly.reduce(function(collectObj, currR) {
            ...
            if (!initiatorTypeData) {
              initiatorTypeData = fileTypeData.initiatorType[
                currR.initiatorType
              ] = {
              ...
              function(tbody) {
                Object.keys(output).forEach(function(key, i) {
                  var fileTypeData = output[key],
                    ...
                    firstinitiatorTypeKey =
                      fileTypeData.initiatorType[initiatorTypeKeys[0]],
                    rowspan = initiatorTypeKeys.length;

                  var tr = dom.newTag('tr', {
                    class:
                      'file-type-row ' +
                      (fileTypeData.fileType || 'other') +
                      '-light',
                  });
                  ...
                  [
                    ...
                    if (i < 4 && initiatorTypeKeys.length > 1) {
                      ...
                      settings['class'] =
                        (initiatorTypeKeys[0] || 'other') + '-light';
                    }
                  ...
                  initiatorTypeKeys
                    .slice(1)
                    .forEach(function(initiatorTypeKey) {
                      var initiatorTypeData =
                        fileTypeData.initiatorType[initiatorTypeKey];
                      var tr2 = dom.newTag('tr', {
                        class:
                          'initiator-type-more ' +
                          (initiatorTypeKey || 'other') +
                          '-light',
                      });
                      tr2.appendChild(
                        dom.newTag('td', {
    ...
    8: [
      function(require, module, exports) {
        ...
        if (
          ...
          data.resources = window.performance.webkitGetEntriesByType(
            'resource',
          );
        ...
        if (window.performance.timing) {
          ...
          alert(
            'Oups, looks like this browser does not support performance timing',
          );
          isValid = false;
          return;
        }
        ...
        data.allResourcesCalc = data.resources
          ...
          .map(function(currR, i, arr) {
            ...
            if (currR.requestStart) {
              ...
              currRes.requestDuration =
                currR.responseStart - currR.requestStart;
            }
        ...
        data.requestsOnly = data.allResourcesCalc.filter(function(currR) {
          return (
            currR.name.indexOf('http') === 0 && !currR.name.match(/js.map$/)
          );
        });

        //get counts
        data.initiatorTypeCounts = helper.getItemCount(
          data.requestsOnly.map(function(currR, i, arr) {
        ...
        data.requestsOnly.forEach(function(currR) {
          var entry =
            data.requestsByDomain.filter(function(a) {
          ...
          currR.duration =
            entry.duration || currR.responseEnd - currR.startTime;
        ...
        data.currAndSubdomainRequests = data.requestsOnly.filter(function(
          domain,
        ) {
          return (
            domain.domain
        ...
        data.crossDocDomainRequests = data.requestsOnly.filter(function(
          domain,
        ) {
        ...
        if (data.allResourcesCalc.length > 0) {
          ...
          data.average = Math.floor(
            data.slowestCalls.reduceRight(function(a, b) {
              if (typeof a !== 'number') {
        ...
        */
        ...
        helper.getRandomColor = function(
          ...
          for (var i = 0; i < 6; i++) {
            r = Math.floor(i / 2);
            color += range[r].split('')[
              Math.floor(Math.random() * range[r].length)
            ];
          }
          return color;
        };
        ...
        var getColourVariation = function getColourVariation(
          hexColour,
          variation,
        ) {
          var r = (parseInt(hexColour.substr(1, 2), 16) + variation).toString(
              16,
            ),
        ...
        helper.getInitiatorOrFileTypeColour = function(
          initiatorOrFileType,
          fallbackColour,
          variation,
        ) {
      ...
      function(require, module, exports) {
        ...
        */
        ...
        var initHolderEl = function initHolderEl() {
          ...
          if (!outputHolder) {
            outputHolder = dom.newTag('div', {
              id: 'perfbook-holder',
            });
            outputContent = dom.newTag('div', {
              id: 'perfbook-content',
            });
        ...
        var getMetrics = function getMetrics() {
          return {
            ...
            total:
              data.perfTiming.loadEventEnd - data.perfTiming.navigationStart,
            timeToFirstByte:
              data.perfTiming.responseStart - data.perfTiming.navigationStart,
            domContentLoading:
              data.perfTiming.domContentLoadedEventStart -
              data.perfTiming.domLoading,
            domProcessing:
              data.perfTiming.domComplete - data.perfTiming.domLoading,
          };
        };
    ...
    13: [
      function(require, module, exports) {
        ...
        var createWedge = function createWedge(
          id,
          size,
          startAngle,
          percentage,
          ...
          var d =
            ...
            radius +
            ',' +
            radius + // Draw an arc of radius r
            ' 0 ' +
            big +
            ' 1 ' + // Arc details...
            x2 +
          ...
          path.addEventListener('mouseenter', function(evt) {
            ...
            evt.target.ownerDocument.getElementById(
              evt.target.getAttribute('id') + '-table',
          ...
          if (percentage > 10) {
            ...
            if (labelAngle < Math.PI) {
              wedgeLabel.setAttribute(
                'x',
                x3 - svg.getNodeTextWidth(wedgeLabel),
              );
            } else {
        ...
        pieChartHelpers.createPieChart = function(data, size) {
          ...
          data.forEach(function(dataObj) {
            var wedgeData = createWedge(
              dataObj.id,
              size,
              startAngle,
              dataObj.perc,
              dataObj.label + ' (' + dataObj.count + ')',
              dataObj.colour || helper.getRandomColor(),
            );
    ...
    15: [
      function(require, module, exports) {
        ...
        */
        ...
        var getOutputIFrame = require('../helpers/iFrameHolder')
          .getOutputIFrame;
    ...
    17: [
      function(require, module, exports) {
        ...
        */
        ...
        waterfall.setupTimeLine = function(
          durationMs,
          blocks,
          marks,
          lines,
          ...
          var unit = durationMs / 100,
            ...
            maxMarkTextLength =
              marks.length > 0
                ? marks.reduce(function(currMax, currValue) {
                    return Math.max(
                      typeof currMax == 'number' ? currMax : 0,
                      svg.getNodeTextWidth(svg.newTextEl(currValue.name, '0')),
                    );
                  })
                : 0,
          ...
          var timeLineLabelHolder = svg.newEl('g', {
            class: 'labels',
          });
          ...
          var onRectMouseEnter = function onRectMouseEnter(evt) {
            ...
            var xPosEnd =
              targetRect.x.baseVal.valueInSpecifiedUnits +
          ...
          var createRect = (function(_createRect) {
            var _createRectWrapper = function createRect(
              _x,
              _x2,
              _x3,
              _x4,
            ...
            var rect = svg.newEl('rect', {
              ...
              y: y,
              class:
                (segments && segments.length > 0 ? 'time-block' : 'segment') +
                ' ' +
                (cssClass || 'block-undefined'),
            });
            if (label) {
              rect.appendChild(
                svg.newEl('title', {
                ...
                if (segment.total > 0 && typeof segment.start === 'number') {
                  rectHolder.appendChild(
                    createRect(
                      ...
                      segment.name +
                        ' (' +
                        Math.round(segment.start) +
                        'ms - ' +
                        Math.round(segment.end) +
                        'ms | total: ' +
                        Math.round(segment.total) +
                        'ms)',
                    ),
                  );
                }
          ...
          var createTimeWrapper = function createTimeWrapper() {
            var timeHolder = svg.newEl('g', {
              class: 'time-scale full-width',
            });
            for (
              var i = 0, secs = durationMs / 1000, secPerc = 100 / secs;
              i <= secs;
              i++
            ) {
          ...
          barsToShow.forEach(function(block, i) {
            ...
            var blockLabel = svg.newTextEl(
              block.name + ' (' + Math.round(block.total) + 'ms)',
              y + (block.segments ? 20 : 17),
            );

            if (
              (block.total || 1) / unit > 10 &&
              svg.getNodeTextWidth(blockLabel) < 200
            ) {
              blockLabel.setAttribute('class', 'inner-label');
              blockLabel.setAttribute(
                'x',
                (block.start || 0.001) / unit + 0.5 + '%',
              );
              blockLabel.setAttribute('width', blockWidth / unit + '%');
            } else if ((block.start || 0.001) / unit + blockWidth / unit < 80) {
              blockLabel.setAttribute(
                'x',
                (block.start || 0.001) / unit + blockWidth / unit + 0.5 + '%',
              );
            } else {
              blockLabel.setAttribute(
                'x',
            ...
            blockLabel.style.opacity = block.name.match(/js.map$/)
              ? '0.5'
              : '1';
    ...
    18: [
      function(require, module, exports) {
        ...
        var summaryTilesComponent = _interopRequire(
          require('./components/summaryTiles'),
        );

        var navigationTimelineComponent = _interopRequire(
          require('./components/navigationTimeline'),
        );

        var pieChartComponent = _interopRequire(
          require('./components/pieChart'),
        );

        var tableComponent = _interopRequire(require('./components/table'));

        var resourcesTimelineComponent = _interopRequire(
          require('./components/resourcesTimeline'),
        );

        var legendComponent = _interopRequire(require('./components/legend'));

        var pageMetricComponent = _interopRequire(
          require('./components/pageMetric'),
        );
